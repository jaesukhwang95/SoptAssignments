...
router.post('/:webtoonIdx/:userIdx', async(req, res) => {
    const selectLikeQuery = 'SELECT * FROM likes WHERE webtoonIdx = ? AND userIdx = ?'
    if (!req.params.webtoonIdx || !req.params.userIdx)
    {
        res.status(200).send(defaultRes.successFalse(statusCode.BAD_REQUEST, resMessage.NULL_VALUE))
    }
    else{
        const likes = await db.queryParam_Parse(selectLikeQuery, [req.params.webtoonIdx, req.params.userIdx]);
        if (!likes) {
        ...
        else if(likes.length == 1)
        {
            ...
            const deleteTransaction = await db.Transaction(async(connection) => {
                await connection.query(deleteLikeQuery, [req.params.webtoonIdx, req.params.userIdx]);
        ...
        else
        {
            ...
            const insertTransaction = await db.Transaction(async(connection) => {
                await connection.query(InsertLikeQuery, [req.params.webtoonIdx, req.params.userIdx]);
                await connection.query(updateLikeCountQuery, [req.params.webtoonIdx]);
            })
